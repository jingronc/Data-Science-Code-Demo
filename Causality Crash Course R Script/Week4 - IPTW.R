...
weighteddata<-svydesign(ids = ~ 1, #~0 or ~1 is a formula for no clusters
                        data =mydata, 
                        weights = ~ weight) #specifying sampling weights
...
#compute confidencce interval for treatment coefficient
causalrd<-(betaiptw[2])
